  ...
  return (
    <div className="relative inline-block text-left">
      <button
        ...
        className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center gap-2 transition active:scale-95 shadow-sm ${
          isLight
            ? 'bg-white/15 hover:bg-white/25 text-white border border-white/20'
            : 'bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200'
        } ${className}`}
      >
        <div className="w-6 h-6 rounded-lg bg-orange-500 text-white flex items-center justify-center font-bold text-xs shadow-inner">
          <User size={13} />
        </div>
      ...
      {/* Dropdown Popup */}
      {showDropdown && (
        <div className="absolute left-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-50 animate-fade-in text-right">
          <div className="px-3 py-2 border-b border-slate-100">
            <p className="text-[11px] text-slate-400 font-bold">الحساب المسجل</p>
            <p className="text-xs font-black text-slate-800 dir-ltr">{profile?.phone}</p>
          </div>
