...
async function testBurgerK() {
  console.log("Checking restaurant burger-k...")
  ...
  const { data: menuItems, error: itemErr } = categoryIds.length > 0
    ? await supabase
        .from('menu_items')
        .select('*')
    ...
    : { data: [], error: null }
  console.log("Menu items count:", menuItems?.length, "Error:", itemErr)
